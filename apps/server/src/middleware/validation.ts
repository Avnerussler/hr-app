...
// Custom validation for נוכחות ומעקב יומי
const hoursValidation = Joi.number().min(0).max(24).precision(2).messages({
...
export const schemas = {
    // נוכחות ומעקב יומי validation
    workHours: {
        body: Joi.object({
            employeeId: joiObjectId.objectId().required().messages({
                'any.required': 'Employee ID is required',
            }),
            employeeName: Joi.string().min(1).max(100).required().messages({
            ...
            hours: hoursValidation.required().messages({
            ...
            submittedBy: joiObjectId.objectId().required().messages({
    ...
    formSubmission: {
        body: Joi.object({
            formId: joiObjectId.objectId().required().messages({
    ...
    // Bulk נוכחות ומעקב יומי validation
    bulkWorkHours: {
        body: Joi.object({
            entries: Joi.array()
                .items(
