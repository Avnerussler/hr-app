...
export function useCreatePersonnel(onFieldError?: (field: string, message: string) => void) {
    const queryClient = useQueryClient()
    return useMutation({
        mutationFn: async (body: Partial<Personnel>) => {
            const { data } = await axios.post(`${BASE_URL}/personnel`, body)
            return data
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['personnel'] })
            invalidateCrossEntityQueries(queryClient)
            toaster.success({ title: 'Success', description: 'Personnel created successfully', duration: 5000, closable: true })
        },
        onError: (error) => mapMutationError(error, onFieldError),
    })
}

export function useUpdatePersonnel(onFieldError?: (field: string, message: string) => void) {
    const queryClient = useQueryClient()
    return useMutation({
        mutationFn: async ({ id, body }: { id: string; body: Partial<Personnel> }) => {
            const { data } = await axios.patch(`${BASE_URL}/personnel/${id}`, body)
            return data
        },
        onSuccess: (_, { id }) => {
            queryClient.invalidateQueries({ queryKey: ['personnel'] })
            ...
            invalidateCrossEntityQueries(queryClient)
            toaster.success({ title: 'Success', description: 'Personnel updated successfully', duration: 3000, closable: true })
        },
        onError: (error) => mapMutationError(error, onFieldError),
    })
}
...
export function useDeletePersonnel() {
    ...
    return useMutation({
        mutationFn: async (id: string) => {
            const { data } = await axios.delete(`${BASE_URL}/personnel/${id}`)
            return data
        },
        onSuccess: (_, id) => {
            queryClient.invalidateQueries({ queryKey: ['personnel'] })
            queryClient.removeQueries({ queryKey: ['personnel', 'detail', id] })
            invalidateCrossEntityQueries(queryClient)
            toaster.success({ title: 'Success', description: 'Personnel deleted successfully', duration: 5000, closable: true })
        },
